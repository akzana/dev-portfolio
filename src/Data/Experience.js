import ih from '../assets/career/ih-color-96low-res.jpg';
// import flo from '../assets/career/flo.png';

const experience = [
  {
    title: 'IM/IT - Island Health',
    image: ih,
    position: 'Clinical Applications Analyst',
    year: 'APR 2021 - JUN 2024',
    description:
      'Provided back-end tech support on a variety of Oracle Cerner, Philips, and Microsoft health information applications to >1K health care providers across the region. Addressed user requests in ServiceNow for troubleshooting and following up with the user promptly after solving the technical problem. Took ownership of new builds.'
  },
  {
    title: 'Research & Development - Island Health',
    image: ih,
    position: 'Research Assistant',
    year: '2020 SEPT - 2020 DEC',
    description:
      'Researching the discrepancies of MAiD administration strategies between BC health authorities. Analyzed data from literature to identify trends and patterns in the data. Created a report to present the findings to the team using LucidChart.'
  }
]

export default experience;